...
## 4.11 `CharacterProfileV1`

構造契約は
[`contracts/character-profile-v1.schema.json`](contracts/character-profile-v1.schema.json)
を正とする。runtime validatorはJSON Schemaに加えて、trim後のNFC保存、Unicode
code point長、UTF-8 4 KiB上限、control文字、prompt境界、URL、メールアドレス、
秘密値、運用識別子を検査する。URL/IDの曖昧なASCII tokenは、versionedな既知TLD、
app固有prefix、opaque-token denyとreadable proper-name allowの固定corpusで決定論的に
判定する。

保存先は `CHARACTER_PROFILE_V1`、system-managed revisionは
`CHARACTER_PROFILE_REVISION` とする。profileとrevisionは同一のlock/CAS/単一範囲
writeで更新し、profile JSON内へrevisionを入れない。`SYSTEM_PERSONA` は自動移行
しない。

これらは既存config sheetへの後方互換な休眠key追加であり、sheet列契約は変更
しない。旧・新Apps Script deploymentが共有するScript Propertiesを安全に保つ
ためplatform `SCHEMA_VERSION` は `2026.07.a2` のまま維持し、character profile
自身を `character-profile.v1` で独立してversion管理する。`migrateSchema()` は
不足するdefault行を冪等に追加するが、PR 2では実環境へ実行しない。
