...
| H4-02F | `=1+1` | ユーザーの吹き出しと`conversation_logs.text`に`=1+1`が本文として残り、計算結果の`2`やエラーへ変わらない。assistantの返答は1件だけ増える。 |
...
#### D+1日：00:00〜23:59（日本時間）の都合のよい時間に日記を検証する
...
2. Apps Script画面で、実行する関数として`runDiaryReleaseTest`を選び、
   ...
   同じ実行内で処理し、別のキューイベントには触れない。人間テスト専用関数なので、
   プロジェクトトリガーが0件の場合だけ本番の`DIARY_DUE_TIME`を適用せず、日本時間の
   前日を対象にする。`config`シートの`DIARY_DUE_TIME`は変更しない。
3. 生成した場合は、実行ログの`PR9_TEST_RESULT runDiaryReleaseTest`で
   ...
   `reason=DIARY_TIME_NOT_REACHED`なら人間テスト対応版がデプロイされていないため、
   再実行せず停止する。
...
日記関数は「実行した当日」ではなく前日を対象にするため、D日の会話を同じD日に
実行しても受入テストにはなりません。D+1日の00:00から23:59までに実行します。
D+2日の00:00を過ぎると対象が次の日へ切り替わるため、その場合は別のD日に
H5-01から再試験します。本番の`schedulerJob()`はこの手動テスト例外を使用せず、
`DIARY_DUE_TIME=23:30`以降に当日分の日記をenqueueします。23:30以降の実行を
日付変更前に完了できなかった場合だけ、翌日の最初のscheduler実行が未生成の前日分を
先に起票します。手動テスト関数は引き続き前日を対象にするため、本番スケジュールの
対象日確認には使用しません。
...
2. 修正版デプロイ後、`diagnoseMemoryReleaseGeneration`を1回だけ実行する。この関数は
   queue eventをclaimせず、attempt数、status、cursor、記憶を変更しない。
3. `PR9_TEST_RESULT diagnoseMemoryReleaseGeneration`が次の5値であることを確認する。
   ...
   一致しなければ`resumeMemoryReleaseTest`を実行せず停止する。診断関数も再実行しない。
4. 再試行期限の到来後、`runOperationalHealthCheck`を1回実行し、集計値が次の状態で
   あることを確認する。
...
5. すべて一致した場合だけ`resumeMemoryReleaseTest`を1回実行する。この関数に
   event ID、payload、dedupe keyを入力しない。
6. `PR9_TEST_RESULT resumeMemoryReleaseTest`が次の7値であることを確認する。
...
7. 採用された記憶がある場合だけ本文を画面上で確認し、会話にない事実がないことを
   確認する。本文と各種IDは証跡へ転記しない。
8. `runOperationalHealthCheck`を再実行し、全イベントの`PENDING`、
...
9. すべて合格した場合だけ通常のH5準備へ戻る。H5-01後の新しいbatchでは、
   `runMemoryReleaseTest`の通常成功を改めて確認する。

診断結果、事前集計、手順6、復旧後集計のいずれかが一致しない場合は
再実行せず停止する。
...
11. 日記対象サイクルは、D日に会話し、同じD日の23:30以降にschedulerが動いて
    D日分を1回だけ生成したことを確認する。日付変更前に起票できなかった障害時だけ、
    D+1日の最初のschedulerでD日分が救済されることを確認する。
